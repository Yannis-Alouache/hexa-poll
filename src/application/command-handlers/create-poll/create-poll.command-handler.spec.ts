...
import {
  tomorrow,
  twoDaysFromNow,
} from '../../../../tests/shared/dates.fixture';
...
describe('Create A Poll', () => {
  ...
  it('throws an error if the start date is after the end date', async () => {
    await expect(
      execute(CreatePollFixtures.startDateAfterEndDateCommand()),
    ).rejects.toThrow(StartDateAfterEndDateException);
  });

  it('throws an error if the start date is in the past', async () => {
    await expect(
      execute(CreatePollFixtures.startDateInThePastCommand()),
    ).rejects.toThrow(StartDateInThePastException);
  });
